...
- タスクトレイに常駐します。初回起動時にUACの確認（管理者権限）を承認すると、以降はWindowsサインイン時に自動的に立ち上がります。
...
- 初回起動時、Windowsサインイン時に自動的に立ち上がるようにタスクスケジューラへの登録を試みます。この登録（ログオン時トリガー付きタスクの作成）には**管理者権限が必要**なため、Windows標準のUAC確認ダイアログが表示されます。「はい」を選ぶと登録が完了し、次回以降はサインインすると自動的に起動します。
    - UACで「いいえ」を選んだ場合や、後から登録し直したい場合は、タスクトレイアイコンを右クリックして「**ログオン時起動を登録...**」を選ぶといつでも再試行できます。
    - 登録しなくてもアプリ自体は動作します（その場合は都度手動で起動してください）。
...
- ログは `%USERPROFILE%\.token-recovery-notifier\app.log` に出力されます。動作がおかしいと感じたときはまずこちらを確認してください。5MBを超えると自動的に `app.log.1` へローテーションされ（直前の内容を1世代分保持）、際限なく肥大化しません。

## アンインストール方法

1. ポータブル版を展開したフォルダ内の `Uninstall.cmd` をダブルクリックします（ソースから実行している場合は `npm run uninstall`）。
2. 内容を確認し `y` で続行すると、以下を自動的に行います。
    - 実行中のアプリを停止
    - ログオン時起動タスクの削除（**管理者権限が必要なためUAC確認ダイアログが表示されます**。「はい」を選んでください）
    - 旧バージョンのレジストリRunキー登録が残っていれば削除
    - `%USERPROFILE%\.token-recovery-notifier`（ログファイル・設定ファイル）の削除
3. 最後に、アプリ本体のフォルダ（`dist`や`node_modules`を含むフォルダ）はご自身で削除してください。単なるファイルなので、いつでも安全に削除できます。
...
- **クラッシュ・強制終了からの自動復旧は行いません。** 何らかの理由でアプリのプロセスが落ちた場合、次にWindowsへサインインするまで自動的には再起動しません（定期的な生存監視は行っていません）。動作が止まっていないか気になる場合は、タスクトレイにアイコンが表示されているかをご確認ください。
- **ログオン時起動の登録には管理者権限（UAC）が必要です。** Windowsのタスクスケジューラで「ログオン時」に起動するタスクを作成する都合上、初回起動時にUAC確認ダイアログが表示されます。会社PC等でUAC操作ができない環境では自動起動の登録ができず、都度手動起動が必要になります。
